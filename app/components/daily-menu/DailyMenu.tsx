...
import {weeklyMenuMetadata, days, weeklyMenu} from '../../metadata/weekly-menu';
import {footer} from '../../metadata/footer';

export default function DailyMenu() {
    const [title1, title2, title3] = weeklyMenuMetadata.title.split(' ');
    ...
    <section id="daily-menu" className="py-20 bg-cream">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="inline-block bg-gold/10 px-6 py-2 rounded-full mb-6">
                <span className="text-gold font-semibold text-sm uppercase tracking-wider">{weeklyMenuMetadata.topTitle}</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-playfair font-bold text-charcoal mb-4">
                {title1} <span className="text-gold">{title2}</span> {title3}
            </h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto italic">
                {weeklyMenuMetadata.subtitle}
            </p>
            <div className="mt-6 bg-white/50 backdrop-blur-sm inline-block px-6 py-3 rounded-full shadow-sm">
                ...
                <span className="text-charcoal font-semibold">{weeklyMenuMetadata.week} {currentWeek}</span>
            </div>
        </div>
        ...
        <div className="mb-8">
            <div className="text-center">
                <div className="inline-block bg-gold/10 px-8 py-3 rounded-full">
                    <span className="text-gold font-bold text-2xl">{weeklyMenuMetadata.price}</span>
                </div>
            </div>
            <h3 className="text-3xl font-playfair font-semibold text-center text-charcoal mb-4">
                {weeklyMenuMetadata.menuText} {selectedDay.toLowerCase() === currentDay.toLowerCase() ? weeklyMenuMetadata.today : days[days.indexOf(selectedDay)]}
            </h3>
            <div className="text-center">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {weeklyMenu.find(weekElement => weekElement.week === currentWeek)?.courses[selectedDayNumber].map((course, index) => (
