					...
					<div>
						...
						<ul className="space-y-3 text-gray-300 text-sm">
							...
							<li className="flex items-start">
								...
								<span>{footer.contact.address.street}<br />{footer.contact.address.city} {footer.contact.address.zipCode}, {footer.contact.address.country}</span>
							</li>
							<li className="flex items-start">
								<i className="fas fa-clock text-gold mr-2 w-4 mt-1"></i>
								<div>
									{footer.contact.businessHours.map(hours => (<div key={hours}>{hours}</div>))}
								</div>
							</li>
						</ul>
					</div>
